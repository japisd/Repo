...
var json = require('./steamapplist.json');
// ```
// 
...
// ```
// 
// The code below allows you to find the games owned by the steam ID and return it as a JSON.
// 
// ```js
...
app.get('/steam/game/:steamID/gamelist', function(httpRequest, httpResponse) {
    ...
    var url = 'http://api.steampowered.com/IPlayerService/GetOwnedGames/' +
        'v0001/'+ steamapikey + httpRequest.params.steamID + '&format=json';
...
app.get('/youtube/:gamename/:achievementname', function(httpRequest,httpResponse){youTube.search(httpRequest.params.gamename + " " + httpRequest.params.achievementname, 15, function(error, result) {
  if (error) {
    console.log(error);
  }
  else {
...
app.get('/steam/game/AppList', function(httpRequest, httpResponse) {
    // Calculate the Steam API URL we want to use
...
app.get('/steam/game/AppList/:appids', function(httpRequest, httpResponse) {
    // Calculate the Steam API URL we want to use
	steamappid = httpRequest.params.appids;
	for (index in json.applist.apps.app){
		
			if (json.applist.apps.app[index].appid === Number(steamappid)){
			break;
			}
		
	}	
	httpResponse.send(json.applist.apps.app[index])
...
app.get('/steam/game/:steamid/:appid/ownedgames', function(httpRequest, httpResponse) {
    ...
    request.get(url, function(error, steamHttpResponse, steamHttpBody) {
        httpResponse.setHeader('Content-Type', 'application/json');
		var applist = JSON.parse(steamHttpBody);
		for (index in applist.response.games){
				
			if (parseInt(httpRequest.params.appid) === applist.response.games[index].appid){
				httpResponse.setHeader('Content-Type', 'application/json');
				httpResponse.send(JSON.stringify(applist.response.games[index]));	
			}
